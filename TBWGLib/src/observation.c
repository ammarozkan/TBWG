...
struct ObservingInformation Observe(struct Character* as, struct World* world)
{
	...
	if(as->b.tool == NULL) {
		obsrv.tool.toolCode = 0;
		obsrv.tool.observationPower = 0;
	} else {
		obsrv.tool.toolCode = as->b.tool->toolCode;
		tbwgmemcpy(obsrv.tool.details, as->b.tool->details, sizeof(as->b.tool->details));
		obsrv.tool.observationPower = 2;
	}
